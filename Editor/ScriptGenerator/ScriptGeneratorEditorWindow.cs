...
public class ScriptGeneratorEditorWindow : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        // 绘制 UI
        template = (ScriptTemplateType)EditorGUILayout.EnumPopup("模板类型", template);
        GUILayout.Label("脚本生成目标路径（Assets开始的相对路径）");
        target = EditorGUILayout.TextField(target);
        overwrite = EditorGUILayout.Toggle("是否覆盖原代码文件", overwrite);

        EditorGUILayout.Space();
        EditorGUILayout.LabelField("以下为模板参数", EditorStyles.boldLabel);
        shortClassName = EditorGUILayout.TextField("类名简写", shortClassName);
        classComment = EditorGUILayout.TextField("类注释", classComment);
        ...
        // 帮助框显示控制开关
        showHelpBox = EditorGUILayout.Toggle("显示模板类型说明", showHelpBox);
        // 帮助框内容
        if (showHelpBox)
        {
